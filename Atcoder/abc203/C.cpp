...
typedef LoveLive LL;
const int MAXN = 210014;
typedef LL __QREADTYPE;
...
inline bool check(int& x, LL& val)
{
	if (!(x >= 0 && x <= N))
		return false;
	return f[x].a <= val;
...
void solve()
{
	...
	while (true) {
		...
		if ((l >= 0 && l <= N) && (lastvis >= 0 && lastvis <= N) &&
		    lastvis < l && l <= N && f[l].a <= ans) {
			ans += pre[l] - pre[lastvis];
			lastvis = l;
			//	for(int i = lastvis + 1; i <= l; ++i)
			//		ans += f[i].b;
		}
		else
			break;
