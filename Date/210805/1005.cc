/***
    ...
    Filename:   1005.cc
    Author:     CleanBlue
    Time:       2021-08-05 12:26:08 utc+8
    Site:       cleanblue.moe
...
void solve(){
    ...
    FOR(i,1,n) visted[i] = false;
    FOR(i,1,m) { cin >> b[i]; visted[b[i]] = true; }
    sort(b+1, b+m+1);
    int cnt = 0;
    FOR(i,1,n) if(!visted[i]) a[++cnt] = i;
    int lef = cnt;
    int mostleft = 1;
    int mostright = cnt;
    FOR(i,1,m){
        if(mostleft > cnt) break;
        if(mostright < mostleft || mostright <= 0) break;
        int pos = UB(a+mostleft, a+mostright+1, b[i]) - a;

        int cntleft = pos - mostleft;
        int cntright = mostright - pos + 1;
        if(cntleft == cntright || cntleft + 1 == cntright){
            lef -= cntleft + cntright;
            mostleft += cntleft;
            mostright -= cntright;
        }
        else if(cntleft > cntright){
            int asdfpasdfkoasdfojasde = cntright;
            lef -= cntright + asdfpasdfkoasdfojasde;
            mostleft += asdfpasdfkoasdfojasde;
            mostright -= cntright;
        }
        else{
            int klosrrthuijir = cntleft;
            lef -= cntleft + klosrrthuijir;
            mostleft += cntleft;
            mostright -= klosrrthuijir;
        }
    }
    if(lef == 0) cout << "YES" << ENDL;
    else cout << "NO" << ENDL;
}
