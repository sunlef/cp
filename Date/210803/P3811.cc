/***
    ...
    Filename:   P3811.cc
    Author:     CleanBlue
    Time:       2021-08-04 02:16:33 utc+8
    Site:       cleanblue.moe
...
#define ksm_type LoveLive
ksm_type ksm(ksm_type a, ksm_type k, ksm_type p){
    ksm_type res = 1 % p;
    a = a % p;
    while(k){
        if(k & 1) res = res * a % p;
...
ksm_type getInv(ksm_type a, ksm_type p) { return ksm(a, p-2, p); }
int n, p;
/*----------- SOLVE CASES -----------*/
void solve(){
    cin >> n >> p;
    FOR(i,1,n) cout << getInv(i,p) << ENDL;
}
