/***
    ...
    Filename:   J.cc
    Author:     CleanBlue
    Time:       2021-07-26 13:53:44 utc+8
    Site:       cleanblue.moe
***/
#include <map>
#include <set>
...
double s[SIZE];
bool check(double x, double arr[], int len, int max_len){
    FOR(i,1,max_len) s[i] = s[i-1] + arr[i] - x;
    double minn = INF, maxn = -INF;
    FOR(i,len,max_len) minn = min(minn, s[i-len]), maxn = max(maxn, s[i] - minn);
    return maxn > 0;
}
/*----------- SOLVE CASES -----------*/
void solve(){
