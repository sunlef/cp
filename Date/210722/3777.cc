/***
    ...
    Filename:   3777.cc
    Author:     CleanBlue
    Time:       2021-07-23 20:25:35 utc+8
    Site:       cleanblue.moe
***/
#include <map>
#include <set>
...
void solve(){
    cnt[1] = cnt[0] = 0;
    cin >> n >> s;
    FOr(i,0,n) a[i+1] = (s[i] == 'B');
    FOR(i,1,n) cnt[a[i]] ++; 

    if(cnt[0] & 1 && cnt[1] & 1) {cout << -1 << ENDL; return;}

    VI ans;
    int cntans = 0;
    if(cnt[0] % 2 == 0){
        FOr(i,1,n) if(!a[i]) ++cntans, ans.EB(i), a[i+1] = !a[i+1];
    }
    else FOr(i,1,n) if(a[i]) ++cntans, ans.EB(i), a[i+1] = !a[i+1];
