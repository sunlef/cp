/***
    ...
    Filename:   142.cc
    Author:     CleanBlue
    Time:       2021-08-04 16:45:22 utc+8
    Site:       cleanblue.moe
...
struct trie{
    ...
    std::vector<int> cot;
    trie(int n, int bit) : n(n), c(bit), son(n*bit+1, std::vector<int>(bit, 0)), cot(n*bit+1, false) {}
    trie(int S) : n(SIZE), c(S), son(SIZE, std::vector<int>(S, 0)), cot(SIZE, false) {}
