/***
    ...
    Filename:   143.cc
    Author:     CleanBlue
    Time:       2021-08-04 17:21:15 utc+8
    Site:       cleanblue.moe
...
struct trie{
    ...
    trie(int n, int bit, int S) : S(S), son(n*bit+1, std::vector<int>(S, 0)) {}
    trie(int S) : S(S), son(SIZE, std::vector<int>(S, 0)) {}
    trie(int size, int S) : S(S), son(size, std::vector<int>(S)) {}
    void insert(const int &str) {
        int p = 0;
        DEC(i,30,0){
            int v = str >> i & 1;
            if(son[p][v] == 0) son[p][v] = ++cnt;
            p = son[p][v];
        }
    }
    LL find(const int &str) {
        int p = 0;
        LL res = 0;
        DEC(i,30,0){
            int v = str >> i & 1;
            if(son[p][!v]) {res = res * 2 + 1; p = son[p][!v]; }
            else {res <<= 1; p = son[p][v];}
        }
        return res;
    }
}; 
...
void solve(){
  ...
  /*  FOR(i,1,N) {
    ...
    LL maxx = -LINF;
    FOR(i,1,N) maxx = max(maxx, tr.find(a[i]));
    cout << maxx << ENDL;*/
}
